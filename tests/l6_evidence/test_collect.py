...
def _measurements(out: Path, collector: str) -> dict:
...
def test_collect_without_tools_measures_nothing(
    ...
    assert doc["schemaVersion"] == "adlc-measurements/v1"
    assert doc["collector"] == collector_cls.name
    assert doc["runId"] == "2026-08-19-a1b2"
    assert doc["variant"] == "candidate-a"
    assert doc["measurements"] == [], "a tool that did not run must measure nothing"
...
def test_collect_clears_stale_output_from_a_previous_attempt(
    ...
    stale_measurements.write_text(
        json.dumps({"measurements": [{"metricId": "lcp_ms", "value": 1, "passed": True}]}),
        encoding="utf-8",
    )
    ...
    assert _measurements(evidence_out, collector_cls.name)["measurements"] == []
...
def test_lighthouse_collect_with_a_stubbed_lhci(
    ...
    measured = {m["metricId"]: m for m in doc["measurements"]}
    ...
    assert all(m["artifactSha256"] == lighthouse_sha for m in doc["measurements"])
...
def test_lighthouse_collect_aggregates_worst_case_across_urls(
    ...
    doc = _measurements(evidence_out, "lighthouse")
    lcp = next(m for m in doc["measurements"] if m["metricId"] == "lcp_ms")
...
def test_lighthouse_collect_reports_a_failed_tool(
    ...
    assert {e["cause"] for e in doc["unmeasured"]} == {"tool_timeout"}
...
def test_k6_collect_with_a_stubbed_k6(
    ...
    assert [e["metricId"] for e in doc["unmeasured"]] == ["made_up_k6_metric"]
...
def test_k6_collect_reports_a_missing_script(
    ...
    doc = _measurements(evidence_out, "k6")
    assert doc["measurements"] == []
    assert all("absent.js" in e["reason"] for e in doc["unmeasured"])
...
def test_k6_collect_reports_an_unreadable_summary(
    ...
    doc = _measurements(evidence_out, "k6")
    assert doc["measurements"] == []
    assert {e["cause"] for e in doc["unmeasured"]} == {"output_unreadable"}
...
def test_axe_collect_with_a_stubbed_node(
    ...
    assert [e["metricId"] for e in doc["unmeasured"]] == ["made_up_axe_metric"]
...
def test_axe_collect_surfaces_a_navigation_failure(
    ...
    doc = _measurements(evidence_out, "axe")
    assert doc["measurements"] == []
    assert all(e["status"] == "not_run" for e in doc["unmeasured"])
    assert any("ERR_REFUSED" in e["reason"] for e in doc["unmeasured"])
...
def test_collect_reports_a_missing_target_url(
    ...
    monkeypatch.setattr(f"{module}.target_urls", lambda doc, collector: [])

    collector_cls().collect(run, "candidate-a", evidence_out)
    doc = _measurements(evidence_out, collector_cls.name)
    assert doc["measurements"] == []
    assert all("ADLC_TARGET_URL" in e["reason"] for e in doc["unmeasured"])
